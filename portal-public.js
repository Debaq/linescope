...
class PortalPublic {
    constructor() {
        ...
        this.openAccordion = null; // Variable para controlar qué acordeón está abierto
    ...
    toggleThesisAccordion(profesorIndex) {
        const accordionContent = document.getElementById(`thesis-content-${profesorIndex}`);
        const accordionHeader = document.getElementById(`thesis-header-${profesorIndex}`);
        const arrow = accordionHeader.querySelector('.accordion-arrow');

        // Si hay otro acordeón abierto, cerrarlo
        if (this.openAccordion !== null && this.openAccordion !== profesorIndex) {
            const openContent = document.getElementById(`thesis-content-${this.openAccordion}`);
            const openHeader = document.getElementById(`thesis-header-${this.openAccordion}`);
        ...
        if (isOpen) {
            ...
            this.openAccordion = profesorIndex;
        }
    }
    ...
    renderProfiles() {
        ...
        container.innerHTML = this.filteredProfesores.map((profesor, index) =>
        this.createProfileCard(profesor, index)
        ).join('');
    }

    createProfileCard(profesor, index) {
                                ...
                                ${background.publicaciones?.length > 0 ? `
                                    ...
                                    ` : ''}
                                                ...
                                                ${previousThesis.length > 0 ? `
                                                    ...
                                                    <div class="thesis-accordion-header" id="thesis-header-${index}" onclick="portalPublic.toggleThesisAccordion(${index})">
                                                    <div class="section-title">Tesis Previas (${previousThesis.length})</div>
                                                    <div class="accordion-arrow">▶</div>
                                                    </div>
                                                    <div class="thesis-accordion-content" id="thesis-content-${index}">
                                                    ${previousThesis.map((tesis, tesisIndex) => `
